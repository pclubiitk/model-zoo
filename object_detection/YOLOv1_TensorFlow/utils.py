...
class CustomReshapeLayer(tf.keras.layers.Layer):
  ...
  def call(self, input):
    ...
    class_probs = K.reshape( input[:, :n_class_prob_elements], (K.shape(input)[0],) + tuple([S[0], S[1], C]) )
    ...
    confs = K.reshape(input[:, n_class_prob_elements:n_class_prob_and_prob_object], (K.shape(input)[0],) + tuple([S[0], S[1], B]))
    ...
    boxes = K.reshape(input[:, n_class_prob_and_prob_object:], (K.shape(input)[0],) + tuple([S[0], S[1], B * 4]))
...
def iou(pred_mins, pred_maxes, true_mins, true_maxes):
    global test_done
    if not test_done:
      print("iou funciton : {} | {} | {} | {}".format(pred_mins, pred_maxes, true_mins, true_maxes) )
      test_done=True
