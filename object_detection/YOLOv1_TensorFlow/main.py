...
import os
from utils import My_Custom_Generator, yolo_loss ,CustomLearningRateScheduler ,lr_schedule
...
def main():
    ...
    if json_object['train_val_VOCdevkit_path']=='None' or json_object['test_VOCdevkit_path']=='None':
        print("Please set the test and train dataset path correctly in the config.json file first.")
        return
    ...
    print("Train VOCdevkit path set to {}\n Test VOCdevkit path set to {}".format(train_vocdevkit,test_vocdevkit))
    ...
    for year, image_set in sets:
        ...
        with open(os.path.join('{}/VOC{}/ImageSets/Main/{}.txt'.format(path_to_vocdevkit ,year, image_set)), 'r') as f:
            image_ids = f.read().strip().split()
        with open(os.path.join('{}/{}_{}.txt'.format(path_to_vocdevkit,year, image_set)), 'w') as f:
            for image_id in image_ids:
                f.write('{}/VOC{}/JPEGImages/{}.jpg'.format(path_to_vocdevkit,year, image_id))
