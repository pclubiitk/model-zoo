...
parser.add_argument('--image_size1', type=int, default=64)
parser.add_argument('--image_size2', type=int, default=256)
...
for epoch in range(args.epochs2):
  ...
  for i,data in enumerate(dataloader):
    ...
    if i%10==0:
      print("Epoch [%d/%d] Step[%d/%d] G_loss:%f D_loss:%f KL: %f"%(epoch+1,epochs,i+1,len(dataloader),lossG.item(),lossD.item(),kl.item()) )
