...
def train(config, train_dl, valid_dl, model, optimizer, scheduler, criterion, metric):
    ...
    for epoch in range(config["epochs"]):
        ...
        train_loss, train_metric = train_step(
            model, criterion, train_dl, optimizer, config["device"], metric
        )

        # evaluation step
        model.eval()
        val_loss, val_metric = validation_step(
            model, criterion, valid_dl, config["device"], metric
        )

        scheduler.step()

        print(
            "Epoch [{}/{}, train_loss: {:.4f}, train_iou: {:.4f}, val_loss: {:.4f}, val_iou: {:.4f}".format(
                epoch + 1,
                config["epochs"],
                train_loss,
                train_metric,
