...
class Conv_Block(nn.Module):
    ...
    def forward(self, x):
        out = self.relu(self.conv2(self.conv1(x)))
        skip_connection = out
...
class Upsampling_Block(nn.Module):
    def __init__(self, in_channels, out_channels, padding=0):
        super().__init__()
        # self.upsample = nn.Upsample(scale_factor=2, mode='bilinear')  # has no parameters
        self.conv_transpose = nn.ConvTranspose2d(in_channels, out_channels, kernel_size=2, stride=2)
        self.conv1 = nn.Conv2d(out_channels*2, out_channels, kernel_size=3, padding=padding)
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=padding)
        self.relu = nn.ReLU()
    ...
    def forward(self, x, skip_input):
        ...
        out = self.relu(self.conv2(self.conv1(out)))
        return out
...
class UNet(nn.Module):
    def __init__(self, in_channels, num_classes, features=64, padding=0):
        ...
        self.cb5 = Conv_Block(features*8, features*16, max_pooling=False, padding=padding)
